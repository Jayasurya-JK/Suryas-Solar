...
def fix_calc_links():
    ...
    # Files identified by earlier grep (or just scan all)
    files_to_check = [
    ...
    for filename in files_to_check:
        ...
        # Replace /cost-calculator with /calc
        new_content = content.replace('/cost-calculator', '/calc')
        
        if content != new_content:
            with open(filepath, 'w', encoding='utf-8') as f:
                f.write(new_content)
            print(f"Fixed calc link in: {filename}")
